        ...
        #print(results)
                ...
                if count_value==0:
                    ...
                    if 'yes' in query or 'yep' in query or 'yeah' in query:
                        ...
                        speak("Added!!")
                        dict_tasks.clear()
                        """print("Here is the organized routine for you")
                        speak("Here is the organized routine for you")
                        print("To Encounter your tasks with ease")
                        speak("To Encounter your tasks with ease")
                        #mycursor.execute("select *from tasks")
                        root=Tk()
                        root.geometry("200x200")
                        root.title("MAXX")
                        Label(root,text="Your Automted Routine",font="comicsans 20 bold").pack(padx=10,pady=20)

                        #print("Task No\t\tTask Name\t\tTask deadline")
                        k=1
                        Label(root,text="Task Name").grid(row=0,column=4)
                        Label(root,text="Deadlines").grid(row=1,column=2)
                        for i in range(len(dateslist)):
                            #print(dict_tasks[dateslist[i]][0], "\t\t", dict_tasks[dateslist[i]][1])
                            Label(root,text=dict_tasks[dateslist[i]][0]).grid()
                        root.mainloop()



                        print("Would you like to make any changes to this generated routine??")
                        speak("Would you like to make any changes to this generated routine??")

                        query=Takecommand().lower()
                        if 'yes' in query:
                            operations()
                            speak("Here is your updated schedule")
                            print("Here is your updated schedule")
                            print("Task No\tTask Name\t\tDeadline")
                            for i in range(1,len(dateslist)):
                                print(i,"\t",dict_tasks[dateslist[i]][1], "\t\t", dict_tasks[dateslist[i]][0])
                            print("I have added this automated routine to your tasks list")
                            speak("I have added this automated routine to your tasks list")
                        else:
                            print("Great!I will add this to your tasks list")
                            speak("Great!I will add this to tour tasks list")
                            for x in dateslist:
                                (t_name, t_deadline) = dict_tasks[x]
                                sql = "INSERT INTO tasks values(%s,%s)"
                                val = (t_name, t_deadline)
                        ...
                        # ttk.Button(root, text='', command=calendar_view).pack(padx=10, pady=10)
                       # Button(root, text='schedule tasks', command=dateentry_view).pack(padx=10, pady=10)
                        #root.mainloop()"""
